...
function cinematico_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'cinematico_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function cinematico_theme() {
	return Theme::getInstance();
}

function cinematico_assets() {
	return cinematico_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    cinematico_theme()->register_menus(
        array(
            'header-menu' => esc_html__( 'Header Menu', 'cinematico' ),
            'footer-menu' => esc_html__( 'Footer Menu', 'cinematico' ),
        )
    );
}, 1);
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	cinematico_assets()
		...
		->registerStylesheet( 'cinematico-theme', '/theme/theme.css' );
	cinematico_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		cinematico_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'cinematico_print_color_scheme', 0 );
} else {
	cinematico_assets()
		->registerStyle( 'cinematico-theme', cinematico_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function cinematico_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'cinematico_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'cinematico_after_activation_redirect_url' );
function cinematico_after_activation_redirect_url( $url ) {
...
function cinematico_get_builder_plugin_slug() {
...
function cinematico_theme_plugins( $plugins ) {

	$plugin_slug = cinematico_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'cinematico' ) : __( 'Kubio', 'cinematico' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'cinematico_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === cinematico_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'cinematico' ),
									cinematico_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'cinematico' ),
									cinematico_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'cinematico' ) ); ?>
                        </a>
...
function cinematico_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'cinematico_try_online_url', 10 );

function cinematico_render_header_style() {

	?>
    <base target="_top">
	<?php

	cinematico_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'cinematico_render_header_style', 100 );
