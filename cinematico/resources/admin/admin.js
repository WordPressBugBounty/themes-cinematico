...
(function ($) {
  var $notice_container = $(".cinematico-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.cinematico_admin) {
    return;
  }

  var _window$cinematico_ad = window.cinematico_admin,
      builderStatusData = _window$cinematico_ad.builderStatusData,
      getStartedData = _window$cinematico_ad.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.cinematico-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".cinematico-admin-big-notice").addClass("processing");
      $(".cinematico-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".cinematico-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.cinematico-admin-big-notice");
    });
  }
  ...
  var $notice_root = $notice_container.closest(".cinematico-admin-big-notice");
  var $custom_close_button = $notice_root.find(".cinematico-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".cinematico-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.cinematico_admin.showOverlay = showOverlay;
})(jQuery);
})();
