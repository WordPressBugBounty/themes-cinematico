...
class Flags {
	...
	private function withFlags( $action, $flag = null, $data = null ) {
		...
		if ( $action === 'get' ) {
			...
			$cinematico_flags_defaults = apply_filters( 'kubio/instance-flags-default', array() );

			if ( isset( $cinematico_flags_defaults[ $flag ] ) ) {
				return $cinematico_flags_defaults[ $flag ];
			}
